...
class TrackingFragment : Fragment(R.layout.fragment_tracking){
    ...
    private fun zoomMapDynamically(){
    ...
    private fun endRunAndSaveToDb(){
        map?.snapshot {
            ...
            val averageSpeed = round(distanceInMeters * 36000f / curTimeInMillis ) / 10f
            val dateTimestamp = Calendar.getInstance().timeInMillis
            val caloriesBurned = ( (distanceInMeters / 1000f) * weight).toInt()

            val run = Run(it, dateTimestamp, averageSpeed, distanceInMeters, curTimeInMillis, caloriesBurned)
